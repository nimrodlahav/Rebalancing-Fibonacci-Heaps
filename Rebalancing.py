...
class AVLTree(object):
    ...
    # Helper method that returns the triplet (key, path, parent), Complexity: O(logn)
    def inner_search(self, key, finger=False):
        ...
        if not finger:
            ...
            if curr.is_real_node():
                ...
                while curr is not self.root and curr.key > key:
                    curr = curr.parent
                    path += 1
                    if curr.key < key: # Prevent counting of edge out of path
                        path -= 2
    ...
    @returns: a tuple (x,e) where x is the node corresponding to key (or None if not found),
    and e is the number of edges on the path between the starting node and ending node+1.
    *Complexity: O(logm), when m represents the number of nodes in the subtree, whose root is
    the node from which we will perform a standard search down the tree
    """
    ...
    # Helper method for rebalancing an AVL tree after insertion, Complexity: O(logn)
        ...
        while p is not None:
            ...
            if abs(p.get_bf()) < 2 and prev_height == p.height: # Terminal case: Height has not changed
                ...
                if p.get_bf() == 2:
                    ...
                    elif p.right.get_bf() == -1 or (join and p.right.get_bf() == 0): # Single left rotation
                        self.left_rotation(p.right)
                if join: # Promotion case
                    p.parent.height =- 1
                    p = p.parent
                else: # Terminal case
                    break
    ...
    # Helper method for rebalancing an AVL tree after deletion, Complexity: O(logn)
    def rebalance_post_deletion(self, p):
        while p is not None:
            prev_height = p.height
            p.set_height()
            if abs(p.get_bf()) < 2 and prev_height == p.height: # Terminal\Promotion case: Height has not changed
                if p.parent: # If p is not root, check its parent's updated height
                    prev_p_height = p.parent.height
                    p.parent.set_height()
                    if prev_p_height == p.height:
                        break
                p = p.parent
            elif abs(p.get_bf()) < 2 and prev_height != p.height: # Promotion case: P's height has changed
                p = p.parent
    ...
    # Helper method for insert/finger insert, Complexity: O(logn)
    def inner_insert(self, key, val, finger=False):
        ...
        new_node.height = 0 # Set new node's height
        parent = self.inner_search(key, finger)[2] # finger=True in order to maintain optimal complexity
        new_node.parent = parent # Set new node's parent
        if parent is not None: # Set node's parent child
            ...
            self.root = new_node
        self.size += 1 # Size update
        if self.max is None or key > self.max.key: # Max node update, if necessary
            self.max = new_node
    ...
    and h is the number of PROMOTE cases during the AVL rebalancing
    """
    # Complexity: O(logm), as mentioned for finger_search method
    ...
    # Helper method for finding node's predecessor
    def predecessor(self, node):
        if node.left.is_real_node(): # Go left once, and then right all the way
            left_subtree = AVLTree(node.left)
            return left_subtree.max_node()
        p = node.parent
    ...
    def delete(self, node):
        ...
        if not node.left.is_real_node() and not node.right.is_real_node():
            if node.parent is not None:
                ...
                self.root = AVLNode()
            p = node.parent
            if self.max == node: # Max node update, if necessary
        ...
        # Case 4: Deleted node has two children
            ...
            if node.parent is not None:
                # Update parent's reference to the successor
                if node.key < node.parent.key:  # If it's a left child
    ...
    def join(self, tree2, key, val):
        ...
        # Check if at least one of the trees is empty
        if self.root is None and tree2.root is None:
            ...
            if self.root.height > tree2.root.height: # Self is higher
                higher_tree = self
                lower_tree = tree2
            else: # Tree2 is higher/both heights are equal
                higher_tree = tree2
                lower_tree = self
            curr = higher_tree.root
            if higher_tree.max.key > key: # Higher tree is on the right
                # Search for node with a height less than or equal to lower tree's height
                while curr.height > lower_tree.root.height and curr.left.is_real_node():
                    curr = curr.left
                left_root = lower_tree.root
                # Join both trees
                ...
                if self == lower_tree: # Max node update, if necessary
                    self.max = tree2.max
            else: # Higher tree is on the left
                while curr.height > lower_tree.root.height and curr.right.is_real_node():
                    curr = curr.right
                ...
                if self == higher_tree: # Max node update, if necessary
                    self.max = tree2.max
            self.size += tree2.size + 1 # Size update
            join_node.set_height()
            higher_tree.rebalance_post_insertion(join_node.parent, True) # Rebalance, if necessary

            # Root update, if necessary
            if join_node.height > self.root.height and join_node.height > tree2.root.height:
                self.root = join_node
            elif tree2.root.height > self.root.height:
                self.root = tree2.root
                
    # Helper method for creating new AVL tree
    def create_subtree(self, node):
        subtree = AVLTree(node)
        node.parent = None
        if node.is_real_node():
            subtree.root = node
            subtree.max = subtree.max_node()
        else:
            subtree.root = None
            subtree.max = None
        return subtree
    ...
    """
    def split(self, node):
        left_subtree = self.create_subtree(node.left)
        right_subtree = self.create_subtree(node.right)
        while node.parent is not None:
            if node.parent.right == node: # Perform join operation on the left subtree
                tree_to_join = self.create_subtree(node.parent.left)
                left_subtree.join(tree_to_join, node.parent.key, node.parent.value)
            else: # Perform join operation on the right subtree
                tree_to_join = self.create_subtree(node.parent.right)
